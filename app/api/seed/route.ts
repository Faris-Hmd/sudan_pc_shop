...
const CATEGORY_DATA = {
  PC: {
    imgs: [ "https://images.unsplash.com/photo-1547082299-de196ea013d6?q=80&w=1000"],
    desc: "Custom-built performance workstation with high-airflow chassis.",
    priceRange: [500000, 2500000]
  },
  LAPTOP: {
    imgs: ["https://images.unsplash.com/photo-1496181133206-80ce9b88a853?q=80&w=1000", "https://images.unsplash.com/photo-1525547719571-a2d4ac8945e2?q=80&w=1000"],
    desc: "Powerful portable computing with high-refresh display and thermal mastery.",
    priceRange: [400000, 1800000]
  },
  WEBCAMS: {
    imgs: [ "https://images.unsplash.com/photo-1588702547919-26089e690ecc?q=80&w=1000"],
    desc: "4K Ultra-HD webcam with auto-focus and low-light correction.",
    priceRange: [15000, 85000]
  },
//   HARD_DRIVES: {
//     imgs: ["https://images.unsplash.com/photo-1531492746377-ad60cb3e285d?q=80&w=1000"],
//     desc: "High-capacity mechanical storage for enterprise-grade data backup.",
//     priceRange: [25000, 120000]
//   },
  HEADSETS: {
    imgs: ["https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=1000"],
    desc: "Spatial audio gaming headset with noise-canceling microphone.",
    priceRange: [20000, 150000]
  },
  KEYBOARDS: {
    imgs: ["https://images.unsplash.com/photo-1511467687858-23d96c32e4ae?q=80&w=1000"],
    desc: "Mechanical deck with tactile switches and per-key RGB programming.",
    priceRange: [30000, 130000]
  },
  SPEAKERS: {
    imgs: ["https://images.unsplash.com/photo-1545454675-3531b543be5d?q=80&w=1000", "https://images.unsplash.com/photo-1616763355548-1b606f439f86?q=80&w=1000"],
    desc: "High-fidelity studio monitors with deep bass and clear acoustics.",
    priceRange: [45000, 250000]
  },
  PRINTERS: {
    imgs: ["https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?q=80&w=1000"],
    desc: "High-speed laser printer with wireless cloud printing capabilities.",
    priceRange: [80000, 350000]
  },
  MICROPHONES: {
    imgs: ["https://images.unsplash.com/photo-1590602847861-f357a9332bbc?q=80&w=1000"],
    desc: "Professional condenser microphone for high-quality streaming and podcasts.",
    priceRange: [25000, 180000]
  },
  MONITORS: {
    imgs: ["https://images.unsplash.com/photo-1527443224154-c4a3942d3acf?q=80&w=1000", "https://images.unsplash.com/photo-1551645120-d70bfe84c826?q=80&w=1000"],
...
//   },
  MOUSES: {
    imgs: ["https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?q=80&w=1000"],
    desc: "Lightweight wireless gaming mouse with high-precision optical sensor.",
    priceRange: [12000, 65000]
  }
};

const BRANDS = ["ASUS", "MSI", "Corsair", "Samsung", "Razer", "Intel", "AMD", "Logitech"];

export async function GET() {
  try {
    const categories = Object.keys(CATEGORY_DATA) as (keyof typeof CATEGORY_DATA)[];
    
    // Step 1: Create a list of promises to ensure EACH category has at least one product
    const guaranteePromises = categories.map((catKey) => {
      const config = CATEGORY_DATA[catKey];
      const brand = BRANDS[Math.floor(Math.random() * BRANDS.length)];
      const randomImg = config.imgs[Math.floor(Math.random() * config.imgs.length)];
      const cost = Math.floor(Math.random() * (config.priceRange[1] - config.priceRange[0]) + config.priceRange[0]);

      return addDoc(collection(db, "products"), {
        p_name: `${brand} ${catKey} Elite Series`,
        p_cost: cost,
        p_cat: catKey,
    ...
    // Step 2: Add 10 more random products to fill the inventory
    const extraPromises = Array.from({ length: 10 }).map(() => {
    ...
    return NextResponse.json({ 
      success: true, 
      message: `Database seeded: 1 product per category + 10 random items.` 
    }, { status: 200 });
