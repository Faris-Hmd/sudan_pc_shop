...
export default function HistoryPage() {
  ...
  const stats = useMemo(() => {
    ...
    return orders.reduce((acc, order) => {
      const itemsCount = order.productsList?.reduce((sum, p) => sum + Number(p.p_qu), 0) || 0;
      return {
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href={"/orders" as any} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
              <ArrowLeft size={20} />
            </Link>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-2 rounded-xl shadow-sm">
              <div className="flex flex-col items-center justify-center py-1 border-r border-slate-100 dark:border-slate-800">
                <span className="text-[12px] font-black text-slate-400 uppercase tracking-tighter">Spend</span>
                <p className="text-xs font-black text-slate-900 dark:text-white">
                  {Math.round(stats.totalSpend / 1000)}k <span className="text-[7px] text-blue-500">SDG</span>
                </p>
              </div>
              <div className="flex flex-col items-center justify-center py-1 border-r border-slate-100 dark:border-slate-800">
                <span className="text-[12px] font-black text-slate-400 uppercase tracking-tighter">Units</span>
                <p className="text-xs font-black text-slate-900 dark:text-white">{stats.totalItems}</p>
              </div>
              <div className="flex flex-col items-center justify-center py-1">
                <span className="text-[12px] font-black text-slate-400 uppercase tracking-tighter">Orders</span>
                <p className="text-xs font-black text-slate-900 dark:text-white">{stats.count}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between px-1">
              <h2 className="text-[12px] font-black text-slate-400 uppercase tracking-[0.2em]">Deployment Log</h2>
