...
function CartBtn({ product }) {
  ...
  useEffect(() => {
    ...
    const item = savedCart.find((p) => p.productId === product.productId);
    if (item) {
      setIsInCart(true);
      setQu(item.q.toString());
    }
  }, [product.productId]);

  const getCart = () => JSON.parse(localStorage.getItem("sh") || "[]");

  const updateCart = (quantity, isRemoving = false) => {
    const currentCart = getCart();
    let newCart;

    if (isRemoving) {
      newCart = currentCart.filter((p) => p.productId !== product.productId);
      setIsInCart(false);
      setQu("1");
    } else {
      // Remove old version and add new version with updated quantity
      newCart = currentCart.filter((p) => p.productId !== product.productId);
      newCart.push({ ...product, q: parseInt(quantity) });
      setIsInCart(true);
