...
export default async function ProductTable({
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-slate-50/30 dark:bg-black transition-colors duration-500">
      <div className="p-2 md:p-8 flex flex-col h-full gap-4 md:gap-6 overflow-hidden">
        {/* Header Section */}
        <header className="flex flex-col gap-4 bg-white dark:bg-slate-900 p-4 md:p-6 rounded border border-slate-200 dark:border-slate-800 shadow-sm shrink-0">
        ...
        {/* Table Container - overflow-hidden is key to prevent horizontal scroll */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm grow overflow-hidden flex flex-col rounded transition-all">
          <div className="overflow-y-auto overflow-x-hidden grow">
            <table className="w-full divide-y divide-slate-100 dark:divide-slate-800 table-fixed">
              <thead className="bg-slate-50/50 dark:bg-slate-800/50 sticky top-0 z-10 backdrop-blur-md">
                <tr>
                  {/* Identification: Takes 50-60% of space */}
                  <th className="px-4 md:px-8 py-5 text-left text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] w-[50%] sm:w-[60%]">
                    Identification
                  </th>
                  {/* Pricing: Fixed width */}
                  <th className="px-4 md:px-8 py-5 text-right md:text-left text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] w-28 md:w-40">
                    Pricing
                  </th>
                  {/* Category: Hidden on small screens */}
                  <th className="hidden lg:table-cell px-8 py-5 text-left text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] w-40">
                    Tag
                  </th>
                  {/* Actions: Small fixed width */}
                  <th className="px-4 md:px-8 py-5 text-center text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] w-16 md:w-24">
                    Edit
                  </th>
                </tr>
              </thead>
                ...
                {products?.length > 0 ? (
                  products.map((row) => (
                    <tr key={row.id} className="group hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                      <td className="px-4 md:px-8 py-4 md:py-5 overflow-hidden">
                        <Link
                          href={"/products/" + row.id as any} 
                          className="flex items-center gap-3 md:gap-4 group/item max-w-full"
                        >
                          <div className={`hidden sm:flex p-2.5 rounded-xl transition-all group-hover:scale-110 ${
                            row.isFeatured 
                          ...
                          <div className="flex flex-col min-w-0 truncate">
                            <div className="flex items-center gap-2">
                              <span className="text-sm md:text-base font-black text-slate-900 dark:text-slate-100 truncate group-hover:text-blue-600 transition-colors">
                                {row.p_name}
                              </span>
                              {row.isFeatured && (
                      ...
                      <td className="px-4 md:px-8 py-4 md:py-5 text-right md:text-left">
                        <span className="text-sm md:text-base font-black text-slate-900 dark:text-white whitespace-nowrap">
                      ...
                      <td className="hidden lg:table-cell px-8 py-4 md:py-5">
                        <span className="px-3 py-1 rounded-lg text-[10px] font-black bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-slate-700 uppercase tracking-widest truncate block w-fit">
